...
public class Scanner {
	...
	public List<Token> scan(String program) {
		StringTokenizer tokenizer = new StringTokenizer(program, Token.DELIMITADOR_DE_TOKEN);
	...
	private Token getToken(String token) {
		Token ret = null;
		if(Regex.ehNumero(token)) {
			ret = new Token(TokenType.NUM, token);
		}
		else if(Regex.ehOperador(token)) {
			ret = new Token(Regex.pegarTokenOperador(token), token);
		} else if (Regex.ehID(token)) {
			ret = new Token(TokenType.ID, token);
		} else {
			throw new lexamaError("Unexpected character: "+token);
		}
		return ret;
	}
}
